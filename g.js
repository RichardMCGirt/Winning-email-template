(async () => {
  // === CONFIG: make sure these are available ===
  const baseId  = window.baseId  || "appK9gZS77OmsIK50";   // <-- your base
  const tableId = window.tableId || "tblQo2148s04gVPq1";   // <-- your "Bids" table
  const apiKey  = window.airtableApiKey || "patCnUsdz4bORwYNV.5c27cab8c99e7caf5b0dc05ce177182df1a9d60f4afc4a5d4b57802f44c65328";       // <-- provide if not already global
  const bidName = "Compass Hotel";                          // <-- the Bid Name to match

  // Checkbox field names to try (first one that exists will be set true)
  ...
  async function listByBidName(name) {
    // Try common name fields for the bid
  ...
  // --- main ---
  if (!baseId || !tableId || !apiKey) throw new Error("Missing baseId/tableId/apiKey (set window.baseId, window.tableId, window.airtableApiKey).");

  const { fieldMatched, records } = await listByBidName(bidName);
  if (!fieldMatched || records.length === 0) {
    console.warn(`[EmailFlag] No records matched Bid Name "${bidName}".`);
    return;
  }
